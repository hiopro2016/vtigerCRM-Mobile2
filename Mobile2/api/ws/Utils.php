...
class Mobile_WS_Utils {
	...
	* 获取相关的模块[id,name]
	*/
	static function getAllRelations($sourceModule, $selected = true, $onlyActive = true) {
		global $adb;
        ...
        $query .= ' AND vtiger_relatedlists.name NOT IN ( '.implode(',',$skipReltionsList).' ) ORDER BY sequence'; // TODO: Need to handle entries that has related_tabid 0

        $result = $adb->pquery($query, array(getTabid($sourceModule)));

        $rows = array();
        for($i=0; $i<$adb->num_rows($result); $i++) {
			$row = $adb->query_result_rowdata($result, $i);
			$rows[] = array('id'=>$row['related_tabid'],'name'=>$row['modulename']);
		}
		return $rows;
	}
	/**
