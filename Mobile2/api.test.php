...
// $mobileAPITestController->doLoginAndFetchModules('admin','123654');

//$mobileAPITestController->doListModuleRecords('Contacts'); //ok
$mobileAPITestController->doLogin('admin', '123654');
// $mobileAPITestController->doListCustomView('Campaigns');
//$mobileAPITestController->doListModuleRecords('Potentials');
//
// $mobileAPITestController->doFetchModuleFilters('Campaigns');
...
// $mobileAPITestController->doListModuleRecords('Campaigns');
// $mobileAPITestController->doRecordComment('11x489');
//$mobileAPITestController->doAddRecordComment('11x67','aasdsadsadsa');
//$mobileAPITestController->doModuleFilter();
// $mobileAPITestController->doSalesFunnel('all');

// $mobileAPITestController->doFetchRecord('30x0', true);
...
//$mobileAPITestController->doSync('Project');//, 0, 1277646523, 'public');//, 0, 1277234885);// 1271240542);
...
// $mobileAPITestController->doRelatedRecordsWithGrouping('1x20', 'Contacts', 0);
//$mobileAPITestController->doDeleteRecords(array('1x198', '18x198'));

//$mobileAPITestController->doHistory('Accounts','11x489');

//$mobileAPITestController->doFetchRecord('16x196', false);

class Mobile_API_TestController {
	
	private $URL = 'http://127.0.0.1/modules/Mobile2/api.php';
	private $userid;
	private $session ;
	private $listing;
	...
	function doPost($parameters, $printResponse = false) {
		...
		if($printResponse) var_dump($response);
...
//		$responseJSON = $this->doPost($parameters, true);
	...
	function doHistory($module, $record='') {
		$parameters = array(
			...
			'record' => $record,
			'mode'   => 'Private' , // Private (not supported yet)
		);
		$responseJSON = $this->doPost($parameters, true);
	}
